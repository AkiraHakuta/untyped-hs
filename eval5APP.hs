...
eval1 ctx (TmVar n)                             = case getbinding ctx n of 
                                                             ...
                                                             _                      -> Nothing  
eval1 ctx (TmApp t1 t2) | isJust (eval1 ctx t1) = do {t1' <- eval1 ctx t1; return (TmApp t1' t2)}
eval1 ctx (TmApp t1 t2) | isJust (eval1 ctx t2) = do {t2' <- eval1 ctx t2; return (TmApp t1 t2')} 
